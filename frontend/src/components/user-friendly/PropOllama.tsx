...
const PropOllama: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      type: 'assistant',
      content: 'Welcome to PropOllama Quantum Neural Interface! I\'m powered by 47 neural networks and quantum processing. I can analyze props, predict outcomes, and provide real-time market insights. How can I assist your betting strategy today?',
      timestamp: new Date()
    }
  ]);
  ...
  const Button = ({ label, onClick, variant = 'primary', className = '', icon = null, disabled = false }) => {
    const variants = {
      primary: 'bg-gradient-to-r from-electric-500 to-neon-blue text-black font-bold hover:shadow-neon',
      secondary: 'bg-gray-700/50 hover:bg-gray-600/50 text-white border-2 border-gray-600 hover:border-gray-500',
    };
    ...
    <div className="space-y-8 animate-slide-in-up h-full flex flex-col">
      ...
      <div className="quantum-card rounded-3xl p-10 text-center">
        ...
        <div className="grid grid-cols-4 gap-6">
          <div>
            <div className="text-2xl font-bold text-electric-400 font-cyber">{realTimeData.neuralNetworks}</div>
            <div className="text-gray-400 font-mono text-sm">Neural Networks</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-purple-400 font-cyber">{realTimeData.neuralIQ}</div>
            <div className="text-gray-400 font-mono text-sm">Neural IQ</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-green-400 font-cyber">{realTimeData.accuracy}%</div>
            <div className="text-gray-400 font-mono text-sm">Prediction Accuracy</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-blue-400 font-cyber">{realTimeData.responseTime}</div>
